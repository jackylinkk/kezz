# -*- coding: utf-8 -*-
"""
可转债波段交易分析系统 v2.8 真实数据版 + 三大核心优化 + 图表功能
改进点1: 正股分析深度增强 - 关联正股驱动能力
改进点2: 事件风险判断精细化 - 强赎进度量化
改进点3: 量能分析精细化 - 结合价格位置分析
新增功能: 图表可视化增强 - 添加买卖点位标记和成交量颜色区分
优化：修复HTML报告中的四维共振综合评分显示问题
优化细节：机构资金流出vs量能健康矛盾、交易触发条件、小盘债弹性量化
"""
...
print("可转债波段交易分析系统 v2.8 真实数据版 + 图表功能".center(70, "="))
print("改进1: 正股分析深度增强 - 关联正股驱动能力".center(70, " "))
print("改进2: 事件风险判断精细化 - 强赎进度量化".center(70, " "))
print("改进3: 量能分析精细化 - 结合价格位置分析".center(70, " "))
print("新增: 图表可视化增强 - 添加买卖点位和成交量颜色".center(70, " "))
print("优化: 三大核心改进集成到原有系统".center(70, " "))
print("优化: 修复HTML报告评分显示，添加细节优化".center(70, " "))
    ...
    def generate_bond_report(self, bond_info, analysis_results, html_filename="bond_analysis_report.html"):
        ...
        try:
            ...
            <head>
                ...
                <title>转债波段分析报告 v2.8 - {bond_info.get('名称', '未知')}</title>
                {self.css_style}
            </head>
            <body>
                <div class="container">
                    <div class="header">
                        <h1>📊 可转债波段分析报告 v2.8 + 图表增强版 + 优化细节</h1>
                        <div class="subtitle">
                            深度增强版 | {bond_info.get('名称', '未知')} ({bond_info.get('转债代码', '未知')}) - {current_time.strftime('%Y年%m月%d日 %H:%M:%S')}
                        </div>
                    </div>
            ...
            html_content += f"""
                    <div class="section">
                        <h2 class="section-title">⚠️ 风险提示 (深度增强 + 优化细节)</h2>
                        <div class="recommendation">
                            <p>1. 本报告基于深度增强分析，包含正股驱动、量能位置、事件风险等多维度评估</p>
                            <p>2. 优化细节：机构资金流出但抛压不重时，可能是散户接盘或机构调仓，市场承接力尚可</p>
                            <p>3. 优化细节：明确交易触发条件 - 连续2根30分钟K线收于关键位置上方，且量比>1.2视为企稳</p>
                            <p>4. 优化细节：量化小盘债弹性 - 剩余规模小于3亿的转债平均日内振幅4.2%，高于市场均值2.8%</p>
                            <p>5. 深度量能分析：结合价格位置判断量能意义（支撑位缩量 vs 突破位缩量）</p>
                            <p>6. 事件风险精细化：强赎进度量化、下修可能性评估</p>
                            <p>7. 新增图表功能：可视化展示价格走势与买卖点位，成交量颜色区分买卖</p>
                            <p>8. 修复：HTML报告中四维共振综合评分显示问题</p>
                            <p>9. 投资有风险，入市需谨慎，建议采用分批建仓、动态止损策略</p>
                            <p>10. 关注市场风险变化，及时调整投资策略</p>
                        </div>
                    </div>
                    ...
                    <div class="footer">
                        <p>可转债波段交易分析系统 v2.8 - 深度增强版 + 图表功能增强 + 优化细节</p>
                        <p>改进点：1.正股驱动深度分析 2.事件风险精细化 3.量能位置分析 4.图表可视化增强 5.修复评分显示 6.添加优化细节</p>
...
def analyze_swing_top10():
    """分析波段推荐前10名 (10线程优化版) - 深度增强版"""
    print("\n" + "="*60)
    print("波段推荐前10名 (10线程深度优化分析)")
    print("="*60)
    print("正在扫描全市场转债波段机会...")
    ...
    multi_analyzer = MultiThreadAnalyzer(max_workers=10)
    
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_bond = {
            executor.submit(multi_analyzer.analyze_single_bond, (code, data)): code
            for code, data in bonds_to_process
        }

        for i, future in enumerate(as_completed(future_to_bond), 1):
            print(f"  进度: {i}/{len(bonds_to_process)}", end='\r')
            try:
    ...
    # 排序取 Top 10
    if results:
        top10 = sorted(results, key=lambda x: x['swing_score'], reverse=True)[:10]
        
        print(f"\n波段推荐前10名 (四维深度共振分析):")
        print("="*130)
        print(f"{'排名':<4} {'名称':<12} {'代码':<10} {'波段分':<8} {'买入分':<8} {'价格':<8} {'溢价率':<8} {'量比':<8} {'正股分':<8} {'事件风险':<8} {'波段类型':<8} {'幅度':<8}")
        print("-"*130)
        
        for i, bond in enumerate(top10, 1):
            swing_type_cn = "上涨" if bond['swing_type'] == 'up' else "下跌" if bond['swing_type'] == 'down' else "未知"
            print(f"{i:<4} {bond['name']:<12} {bond['code']:<10} {bond['swing_score']:<8.1f} {bond['buy_score']:<8.1f} {bond['price']:<8.1f} {bond['premium']:<8.1f}% {bond['volume_ratio']:<8.2f} {bond.get('stock_score', 0):<8.1f} {bond['event_risk']:<8} {swing_type_cn:<8} {bond.get('amplitude', 0):<8.1f}%")
    else:
        print("未找到符合条件的转债")

def analyze_multifactor_top10():
    """分析波段多因子共振前10名 (10线程四维深度共振增强版)"""
    print("\n" + "="*60)
    print("波段多因子共振前10名 v2.8 (10线程深度优化分析)")
    print("="*60)
    print("正在分析多因子深度共振机会...")
    ...
    def analyze_single_bond_multifactor(args):
        ...
        try:
            ...
            price_data_with_indicators = analyzer.calculate_swing_indicators(price_data)
            
            swings, _ = analyzer.analyze_swing_structure(price_data_with_indicators)
            
            current_price = safe_float_parse(data.get('最新价', data.get('债现价', 0)))
            if current_price > 1000: current_price /= 10
            
            volume_analysis = analyzer.analyze_volume_structure_deep(price_data_with_indicators, current_price, swings)
            ...
            stock_analysis = bond_info.get('正股分析', {})
            event_risk = bond_info.get('事件风险等级', 'unknown')
            ...
            buy_signals = analyzer.generate_buy_signals(price_data_with_indicators, swings, 
                                                       price, bond_size, volume_analysis, stock_analysis, bond_info)
            
            buy_score, _ = analyzer.calculate_swing_score(buy_signals, 'buy', volume_analysis, stock_analysis, bond_info)
            ...
            if buy_score >= 50:
                resonance_count += 1
                resonance_details.append("技术")
            
            volume_ratio = volume_analysis.get('volume_ratio', 1.0)
            if volume_ratio > 1.2:
                resonance_count += 1
                resonance_details.append("量能")
            
            stock_score = stock_analysis.get('driving_score', 0)
            if stock_score > 50:
                resonance_count += 1
                resonance_details.append("正股")
            
            if event_risk == 'low':
                resonance_count += 1
                resonance_details.append("事件")
            
            if resonance_count >= 2:
                resonance_type = "四维深度" if resonance_count >= 4 else "三维深度" if resonance_count >= 3 else "二维深度"
                
                bond_name = data.get('债券简称', f"转债{code}")
                
                return {
                    'code': code,
                    'name': bond_name,
                    'price': price,
                    'premium': safe_float_parse(data.get('转股溢价率', 0)),
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_bond = {
            executor.submit(analyze_single_bond_multifactor, (code, data)): code
            for code, data in bonds_to_process
        }
    ...
    if results:
        top10 = sorted(results, key=lambda x: x['resonance_score'], reverse=True)[:10]
        
        print(f"\n波段多因子深度共振前10名 (10线程优化分析):")
        print("="*130)
        print(f"{'排名':<4} {'名称':<12} {'代码':<10} {'共振分':<8} {'共振类型':<10} {'共振数':<4} {'买入分':<8} {'价格':<8} {'溢价率':<8} {'量比':<8} {'正股分':<8} {'事件风险':<8} {'共振因子':<12}")
        print("-"*130)
        
        for i, bond in enumerate(top10, 1):
            resonance_factors = ", ".join(bond['resonance_details'])
            print(f"{i:<4} {bond['name']:<12} {bond['code']:<10} {bond['resonance_score']:<8.1f} {bond['resonance_type']:<10} {bond['resonance_count']:<4} {bond['buy_score']:<8.1f} {bond['price']:<8.1f} {bond['premium']:<8.1f}% {bond['volume_ratio']:<8.2f} {bond['stock_score']:<8.1f} {bond['event_risk']:<8} {resonance_factors:<12}")
    else:
        print("未找到多因子深度共振机会")

def analyze_single_bond_swing():
    """分析单个转债波段交易机会 (支持HTML报告输出) - 深度增强版 + 图表功能"""
    print("\n" + "="*60)
    print("单个转债波段分析 v2.8 (深度增强版 + 图表功能增强 + 优化细节)")
    print("="*60)
    ...
    data_source = BondDataSource()
    analyzer = SwingTradingAnalyzer()
    point_analyzer = SwingTradePointAnalyzer()
    ...
    # 生成买卖信号
    buy_signals = analyzer.generate_buy_signals(price_data_with_indicators, swings, 
                                               current_price, bond_info['剩余规模(亿)'],
                                               volume_analysis, stock_analysis, bond_info)
    sell_signals = analyzer.generate_sell_signals(price_data_with_indicators, swings, current_price)
    
    # 计算得分
    buy_score, buy_details = analyzer.calculate_swing_score(buy_signals, 'buy', volume_analysis, stock_analysis, bond_info)
    sell_score, sell_details = analyzer.calculate_swing_score(sell_signals, 'sell')
    
    # 获取交易建议
    advice = analyzer.get_trading_advice(buy_score, sell_score, current_price, 
                                        swings, bond_info['剩余规模(亿)'], bond_info,
                                        volume_analysis, stock_analysis)
    
    # 显示基本信息
    print(f"\n📊 基本信息:")
    print(f"  名称: {bond_info['名称']}")
    print(f"  代码: {bond_info['转债代码']}")
    print(f"  价格: {bond_info['转债价格']}元")
    print(f"  溢价率: {bond_info['溢价率(%)']}%")
    print(f"  剩余规模: {bond_info['剩余规模(亿)']}亿")
    print(f"  转股价值: {bond_info.get('转股价值', 0):.2f}元")
    
    # 显示事件风险 (增强版)
    print(f"\n⚠️ 事件风险分析 (增强版):")
    print(f"  等级: {bond_info.get('事件风险等级', 'unknown')}")
    print(f"  描述: {bond_info.get('事件风险描述', '')}")
    print(f"  建议: {bond_info.get('事件风险建议', '')}")
    ...
    if stock_analysis:
        ...
        print(f"  评估: {stock_analysis.get('bond_driving_assessment', '')}")
        print(f"  MA20: {'站上' if stock_analysis.get('above_ma20') else '跌破'}")
        print(f"  RSI: {stock_analysis.get('stock_rsi', 50):.1f} ({stock_analysis.get('rsi_status', '未知')})")
        print(f"  量比: {stock_analysis.get('volume_ratio', 1.0):.2f}")
    
    # 显示量能结构 (深度增强)
    print(f"\n📊 量能深度结构分析:")
    print(f"  量比: {volume_analysis['volume_ratio']:.2f} ({volume_analysis['volume_status']})")
    print(f"  模式: {volume_analysis['pattern']}")
    print(f"  量价分析: {volume_analysis['volume_price_analysis']}")
    print(f"  位置分析: {volume_analysis['position_analysis']}")
    print(f"  建议: {volume_analysis['suggestion']}")
    
    # 显示波段分析结果
    print(f"\n🎯 波段分析结果:")
    print(f"  发现波段数量: {len(swings)}个")
    
    if swings:
        latest_swing = swings[-1]
        print(f"  最近波段:")
        print(f"    类型: {'上涨' if latest_swing['type'] == 'up' else '下跌'}")
        print(f"    幅度: {latest_swing['amplitude_pct']:.1f}%")
    
    # 显示买卖信号
    print(f"\n🛒 买入信号 (四维深度评分: {buy_score:.1f}/100):")
    if buy_details:
        for detail in buy_details:
            if "📊" not in detail and "⚠️" not in detail and "🚨" not in detail:
                print(f"  {detail}")
    else:
        print("  暂无买入信号")
    
    print(f"\n🏷️ 卖出信号 (得分: {sell_score:.1f}/100):")
    if sell_details:
    ...
    if stock_analysis and '缺乏上攻引擎' in stock_analysis.get('bond_driving_assessment', ''):
        print(f"\n⚠️ 特别提示: 正股处于弱势整理，转债缺乏上攻引擎，反弹高度受限")
        print(f"💡 建议降低盈利预期，控制仓位")
    
    # 询问是否进行买卖点位分析
    point_analysis = None
    if input("\n是否进行买卖点位详细分析？(y/n): ").strip().lower() == 'y':
        point_analysis = point_analyzer.analyze_buy_sell_points(bond_info, price_data_with_indicators)
        if point_analysis:
            point_analyzer.display_trade_points(point_analysis)
        else:
            print("买卖点位分析失败")
    
    # 询问是否生成HTML报告（包含图表）
    if input("\n是否生成HTML分析报告（包含交互式图表）？(y/n): ").strip().lower() == 'y':
        # 收集分析结果
        analysis_results = {
            'bond_info': bond_info,
            ...
            'price_data': price_data_with_indicators,  # 添加价格数据
            'current_price': current_price
        }
        
        # 如果有买卖点位分析，添加到结果中
        if point_analysis:
            analysis_results['buy_points'] = point_analysis.get('buy_points', [])
            analysis_results['sell_points'] = point_analysis.get('sell_points', [])
        
        # 生成HTML报告
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"{code}_深度分析_{timestamp}.html"
        success = html_generator.generate_bond_report(bond_info, analysis_results, filename)
        if not success:
            print("生成HTML报告失败，请检查错误信息")

def analyze_strategy_top10():
    """分析各策略前10名 (10线程优化版) - 深度增强版"""
    print("\n" + "="*60)
    print("各策略前10名分析 (10线程深度优化筛选)")
    print("="*60)
    ...
    print("3. 小规模策略前10名")
    print("4. 综合策略前10名")
    print("5. 优化筛选策略前10名 (双低+流动性+规模)")
    print("6. 四维深度共振策略前10名 (技术+量能+正股+事件)")
    print("7. 查看数据源状态")
    print("0. 返回")
    ...
    if choice == '1':
        ...
        for _, bond in bond_df.iterrows():
            ...
            premium = safe_float_parse(bond.get('转股溢价率', 0))
            if price > 1000: price /= 10
            if 70 <= price <= 150 and -10 <= premium <= 50 and bond_code:
                bonds_to_process.append((bond_code, bond))
        
        print(f"  初步筛选出 {len(bonds_to_process)} 只转债")
        
        def analyze_double_low_bond(args):
            code, data = args
            try:
                price = safe_float_parse(data.get('最新价', data.get('债现价', 0)))
                if price > 1000: price /= 10
                premium = safe_float_parse(data.get('转股溢价率', 0))
                ...
                # 检查事件风险
                event_risk_info = data_source.data_fetcher.event_analyzer.check_event_risk(
                    code,
                    bond_info={
                        '转债价格': price,
                        '溢价率(%)': premium
                ...
                # 过滤高风险债券
                if event_risk_info[0] == 'high':
                    return None
                
                # 双低策略评分
                score = 200 - double_low
                
                # 事件风险调整
                if event_risk_info[0] == 'low':
                    score += 10
                elif event_risk_info[0] == 'medium':
                    score += 5
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_bond = {
                executor.submit(analyze_double_low_bond, (code, data)): code
                for code, data in bonds_to_process
            }
        ...
        if results:
            top10 = sorted(results, key=lambda x: x['double_low'])[:10]
            
            print(f"\n双低策略前10名 (双低值从低到高):")
            print("="*100)
            print(f"{'排名':<4} {'名称':<12} {'代码':<10} {'双低值':<8} {'价格':<8} {'溢价率':<8} {'事件风险':<8}")
            print("-"*100)
            for i, bond in enumerate(top10, 1):
                print(f"{i:<4} {bond['name']:<12} {bond['code']:<10} {bond['double_low']:<8.1f} {bond['price']:<8.1f} {bond['premium']:<8.1f}% {bond['event_risk']:<8}")
        else:
            print("未找到符合条件的转债")
    
    elif choice == '2':
        print(f"正在获取低溢价策略前10名...")
        ...
        for _, bond in bond_df.iterrows():
            ...
            premium = safe_float_parse(bond.get('转股溢价率', 0))
            if price > 1000: price /= 10
            if price < 130 and premium < 20 and bond_code:
                bonds_to_process.append((bond_code, bond))
        
        print(f"  初步筛选出 {len(bonds_to_process)} 只转债")
        
        def analyze_low_premium_bond(args):
            code, data = args
            try:
                price = safe_float_parse(data.get('最新价', data.get('债现价', 0)))
                if price > 1000: price /= 10
                ...
                # 低溢价策略评分
                score = 100 - premium + (130 - price) * 0.5
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_bond = {
                executor.submit(analyze_low_premium_bond, (code, data)): code
                for code, data in bonds_to_process
            }
        ...
        if results:
            ...
            print(f"\n低溢价策略前10名:")
            print("="*100)
            print(f"{'排名':<4} {'名称':<12} {'代码':<10} {'溢价率':<8} {'价格':<8} {'评分':<8} {'事件风险':<8}")
            print("-"*100)
            for i, bond in enumerate(top10, 1):
                print(f"{i:<4} {bond['name']:<12} {bond['code']:<10} {bond['premium']:<8.1f}% {bond['price']:<8.1f} {bond['score']:<8.1f} {bond['event_risk']:<8}")
        else:
            print("未找到符合条件的转债")
        
    elif choice == '3':
        print(f"正在获取小规模策略前10名...")
        ...
        for _, bond in bond_df.iterrows():
            ...
            price = safe_float_parse(bond.get('最新价', bond.get('债现价', 0)))
            if price > 1000: price /= 10
            size_str = str(bond.get('发行规模', bond.get('剩余规模', '10'))).replace('亿元', '').replace('亿', '').strip()
            try:
                size = float(size_str) if size_str and size_str != 'nan' else 10.0
            except:
                size = 10.0
            
            if price < 140 and size < 5 and bond_code:
                bonds_to_process.append((bond_code, bond, size))
        
        print(f"  初步筛选出 {len(bonds_to_process)} 只转债")
        
        def analyze_small_size_bond(args):
            code, data, size = args
            try:
                price = safe_float_parse(data.get('最新价', data.get('债现价', 0)))
                if price > 1000: price /= 10
                ...
                # 小规模策略评分
                score = 100 - size * 10 + (140 - price) * 0.3 + (30 - premium) * 0.5
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_bond = {
                executor.submit(analyze_small_size_bond, (code, data, size)): code
                for code, data, size in bonds_to_process
            }
        ...
        if results:
            ...
            print(f"\n小规模策略前10名:")
            print("="*100)
            print(f"{'排名':<4} {'名称':<12} {'代码':<10} {'规模':<8} {'价格':<8} {'溢价率':<8} {'评分':<8} {'事件风险':<8}")
            print("-"*100)
            for i, bond in enumerate(top10, 1):
                print(f"{i:<4} {bond['name']:<12} {bond['code']:<10} {bond['size']:<8.1f}亿 {bond['price']:<8.1f} {bond['premium']:<8.1f}% {bond['score']:<8.1f} {bond['event_risk']:<8}")
        else:
            print("未找到符合条件的转债")
        
    elif choice == '4':
        print(f"正在获取综合策略前10名...")
        ...
        for _, bond in bond_df.iterrows():
            ...
            if 70 <= price <= 150 and premium <= 50 and 0.5 <= size <= 50 and bond_code:
                bonds_to_process.append((bond_code, bond, size, price + premium))
        
        print(f"  初步筛选出 {len(bonds_to_process)} 只转债")
        
        def analyze_comprehensive_bond(args):
            code, data, size, double_low = args
            try:
                price = safe_float_parse(data.get('最新价', data.get('债现价', 0)))
                if price > 1000: price /= 10
                premium = safe_float_parse(data.get('转股溢价率', 0))
                ...
                # 综合策略评分
                price_score = max(0, 130 - price) * 1.0
                premium_score = max(0, 30 - premium) * 1.5
                size_score = max(0, 10 - size) * 2.0
                double_low_score = max(0, 140 - double_low) * 1.2
                
                score = price_score + premium_score + size_score + double_low_score
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_bond = {
                executor.submit(analyze_comprehensive_bond, (code, data, size, double_low)): code
                for code, data, size, double_low in bonds_to_process
            }
        ...
        if results:
            ...
            print(f"\n综合策略前10名:")
            print("="*110)
            print(f"{'排名':<4} {'名称':<12} {'代码':<10} {'评分':<8} {'价格':<8} {'溢价率':<8} {'规模':<8} {'双低值':<8} {'事件风险':<8}")
            print("-"*110)
            for i, bond in enumerate(top10, 1):
                print(f"{i:<4} {bond['name']:<12} {bond['code']:<10} {bond['score']:<8.1f} {bond['price']:<8.1f} {bond['premium']:<8.1f}% {bond['size']:<8.1f}亿 {bond['double_low']:<8.1f} {bond['event_risk']:<8}")
        else:
            print("未找到符合条件的转债")
        
    elif choice == '5':
        print(f"正在获取优化筛选策略前10名...")
        ...
        for _, bond in bond_df.iterrows():
            ...
            bond_name = bond.get('债券简称', f"转债{bond_code}")
            
            if (80 <= price <= 150 and 
                -10 <= premium <= 50 and
                0.5 <= size <= 30 and
                not any(word in bond_name for word in ['申购', '配债', '预告', '待上市', '退', 'ST', '*ST']) and
                bond_code):
                bonds_to_process.append((bond_code, bond, size, price, premium, bond_name))
        
        print(f"  初步筛选出 {len(bonds_to_process)} 只转债")
        
        def analyze_optimized_bond(args):
            code, data, size, price, premium, name = args
            try:
                # 检查事件风险
                event_risk_info = data_source.data_fetcher.event_analyzer.check_event_risk(
                    code,
                ...
                # 优化评分
                price_factor = 0
                if price < 110: price_factor = 25
                elif price < 120: price_factor = 20
                elif price < 130: price_factor = 15
                else: price_factor = 10
                
                premium_factor = 0
                if premium < 10: premium_factor = 30
                elif premium < 20: premium_factor = 25
                elif premium < 30: premium_factor = 20
                else: premium_factor = 15
                
                size_factor = 0
                if size < 3: size_factor = 25
                elif size < 5: size_factor = 20
                elif size < 10: size_factor = 15
                else: size_factor = 10
                
                score = price_factor + premium_factor + size_factor
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_bond = {
                executor.submit(analyze_optimized_bond, (code, data, size, price, premium, name)): code
                for code, data, size, price, premium, name in bonds_to_process
            }
        ...
        if results:
            ...
            print(f"\n📊 数据验证:")
            print(f"  债券总数: {len(results)} 只")
            print(f"  平均价格: {sum(b['price'] for b in results)/len(results):.2f}元")
            print(f"  平均溢价率: {sum(b['premium'] for b in results)/len(results):.2f}%")
            print(f"  平均规模: {sum(b['size'] for b in results)/len(results):.2f}亿")
            print(f"  低风险债券占比: {sum(1 for b in results if b['event_risk'] == 'low')/len(results)*100:.1f}%")
        else:
            print("未找到符合条件的转债")
    
    elif choice == '6':
        print(f"正在获取四维深度共振策略前10名...")
...
def analyze_buy_sell_points():
    ...
    # 分析买卖点位
    analysis = point_analyzer.analyze_buy_sell_points(bond_info, price_data)
    
    if analysis:
        point_analyzer.display_trade_points(analysis)
    else:
        print("分析买卖点位失败")

def demo_dynamic_stop_loss():
    """演示动态止损止盈功能"""
    print("\n" + "="*60)
    print("动态止损止盈演示")
    print("="*60)
    
    prices = [100.0, 102.0, 105.0, 103.0, 107.0, 110.0, 108.0, 112.0, 115.0, 113.0]
    
    manager = DynamicStopLossTakeProfit(initial_stop_loss_pct=3.0)
    ...
    manager.set_entry_price(entry_price, atr_value=2.0, volatility_ratio=1.0)
    
    print(f"入场价格: {entry_price}")
    print(f"初始止损位: {manager.stop_loss_price:.2f}")
    print(f"初始跟踪止损: {manager.trailing_stop_price:.2f}")
    print()
    
    for i, price in enumerate(prices, 1):
        status = manager.update_current_price(price)
        
        should_stop_loss, stop_loss_reason = manager.should_stop_loss()
        should_take_profit, take_profit_reason = manager.should_take_profit()
        
        print(f"第{i}次更新 - 当前价格: {price:.2f}")
        print(f"  盈利: {status['profit_pct']:.1f}%")
        print(f"  固定止损: {manager.stop_loss_price:.2f}")
        print(f"  跟踪止损: {manager.trailing_stop_price:.2f}")
        
        if should_stop_loss:
            print(f"  ⚠️ 需要止损: {stop_loss_reason}")
        elif should_take_profit:
            print(f"  🎯 需要止盈: {take_profit_reason}")
        else:
            print(f"  继续持有")
        print()
...
def add_trade_record():
    ...
    try:
        code = input("转债代码: ").strip()
        entry_price = float(input("买入价格: "))
        exit_price = float(input("卖出价格: "))
        entry_date_str = input("买入日期(YYYY-MM-DD): ")
        exit_date_str = input("卖出日期(YYYY-MM-DD): ")
        entry_date = datetime.strptime(entry_date_str, "%Y-%m-%d")
        exit_date = datetime.strptime(exit_date_str, "%Y-%m-%d")
        entry_signal = input("买入信号(如:RSI超卖): ")
        exit_signal = input("卖出信号(如:前高阻力): ")
        shares = int(input("数量(张，默认100): ") or "100")
        
        trade = perf_analyzer.add_trade(
            code, entry_price, exit_price, entry_date, exit_date,
            entry_signal, exit_signal, shares
        )
        print(f"✅ 交易记录已添加: {trade['bond_code']} 盈利 {trade['profit']:.2f}元")
    except Exception as e:
        print(f"❌ 添加失败: {e}")
...
def show_performance_report():
    ...
    if choice == '1':
        perf_analyzer.display_performance_report()
    elif choice == '2':
        perf_analyzer.display_all_trades()
    elif choice == '0':
        return
    else:
        print("无效选择")
...
def main():
    """主程序"""
    print("可转债波段交易分析系统 v2.8 深度增强版 + 图表功能增强 + 优化细节".center(70, "="))
    print("🎯 改进1: 正股分析深度增强 - 关联正股驱动能力".center(70))
    print("🎯 改进2: 事件风险判断精细化 - 强赎进度量化".center(70))
    print("🎯 改进3: 量能分析精细化 - 结合价格位置分析".center(70))
    print("📊 新增: 图表可视化增强 - 添加买卖点位和成交量颜色".center(70))
    print("🔧 优化: 修复HTML报告评分显示，添加优化细节".center(70))
    print("📈 增强: 机构资金矛盾解释、交易触发条件、小盘债弹性量化".center(70))
    
    while True:
        print("\n" + "="*70)
        print("可转债波段交易分析系统 v2.8 - 四维深度共振增强版 + 图表功能增强 + 优化细节")
        print("="*70)
        print("1. 波段推荐前10名 (10线程优化+智能数据源)")
        print("2. 波段多因子共振前10名 (10线程四维深度共振)")
        print("3. 单个转债波段分析 (四维深度共振增强 + 图表增强 + 优化细节)")
        print("4. 买卖点位深度分析 (含动态风控)")
        print("5. 各策略前10名分析 (10线程深度优化筛选)")
        print("6. 绩效统计与分析")
        print("7. 添加交易记录")
        print("8. 查看数据源状态")
        print("9. 动态止损止盈演示")
        ...
        if choice == '1':
            ...
            analyze_single_bond_swing()
        elif choice == '4':
            analyze_buy_sell_points()
        elif choice == '5':
            analyze_strategy_top10()
        elif choice == '6':
            ...
            demo_dynamic_stop_loss()
        elif choice == '0':
            print("\n感谢使用可转债波段交易分析系统 v2.8 - 四维深度共振增强版 + 图表功能增强 + 优化细节！再见！")
            break
        else:
            print("无效选择, 请重新输入")
